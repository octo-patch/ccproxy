...
class TestIsManagedProcess:
    def test_litellm_with_config(self) -> None:
    ...
    def test_mitmweb_not_detected(self) -> None:
    ...
    def test_unrelated_litellm(self) -> None:
...
class TestFindManagedProcesses:
    ...
    def test_finds_litellm(self, mock_iterdir: Mock, mock_cmdline: Mock) -> None:
    ...
    @patch("ccproxy.preflight._read_proc_cmdline")
    @patch("pathlib.Path.iterdir")
    def test_excludes_own_pid(self, mock_iterdir: Mock, mock_cmdline: Mock) -> None:
        own = MagicMock()
    ...
    @patch("ccproxy.preflight._read_proc_cmdline")
    @patch("pathlib.Path.iterdir")
    def test_skips_unmanaged_process(self, mock_iterdir: Mock, mock_cmdline: Mock) -> None:
        proc_dir = MagicMock()
        proc_dir.name = "5555"
        proc_dir.is_dir.return_value = True
        mock_iterdir.return_value = [proc_dir]
...
class TestKillStaleProcesses:
    @patch("os.kill")
    def test_kills_process(self, mock_kill: Mock) -> None:
    ...
    @patch("os.kill")
    def test_already_dead(self, mock_kill: Mock) -> None:
        mock_kill.side_effect = ProcessLookupError
        count = kill_stale_processes([(1234, "litellm .ccproxy/config.yaml")])
        assert count == 1

    @patch("os.kill")
    def test_permission_denied(self, mock_kill: Mock) -> None:
...
class TestRunPreflightChecks:
    def test_clean_system(self, tmp_path: Path) -> None:
    ...
    def test_port_occupied_by_foreign_process(self, tmp_path: Path) -> None:
    ...
    def test_orphan_killed_then_port_freed(self, tmp_path: Path) -> None:
    ...
    def test_mitm_checks_both_ports(self, tmp_path: Path) -> None:
    ...
    def test_no_mitm_checks_main_port_only(self, tmp_path: Path) -> None:
    ...
    def test_does_not_kill_other_instance_processes(self, tmp_path: Path) -> None:
    ...
    def test_port_occupied_unknown_pid(self) -> None:
    ...
    def test_orphan_killed_but_port_still_occupied(self) -> None:
        """Port held by any process → SystemExit (no pattern matches, so no auto-kill)."""
        fake_cmdline = "/usr/bin/litellm --config /home/user/.ccproxy/config.yaml"
        with (
    ...
    def test_config_dir_triggers_wg_cleanup(self, tmp_path: Path) -> None:
...
class TestGetPortPidExtra:
    def test_inode_found_but_no_pid_resolution(self) -> None:
    ...
    def test_tcp_oserror_continues(self) -> None:
    ...
    def test_tcp6_v4mapped_address_match(self) -> None:
        ...
        def fake_open(self: Path, *args: Any, **kwargs: Any) -> StringIO:
            if "tcp6" in str(self):
    ...
    def test_short_tcp_line_skipped(self) -> None:
    ...
    def test_socket_bind_fails_returns_neg1(self) -> None:
...
class TestFindManagedProcessesExtra:
    def test_oserror_on_proc_scan(self) -> None:
    ...
    def test_skips_non_digit_entries(self) -> None:
...
class TestReadProcCmdline:
    def test_reads_real_self(self) -> None:
...
class TestFindInodePids:
    def test_handles_oserror_on_iterdir(self) -> None:
        with patch("pathlib.Path.iterdir", side_effect=OSError("no /proc")):
...
class TestIsUdpPortInUse:
    ...
    def test_detects_bound_udp_port(self) -> None:
    ...
    def test_udp_short_line_skipped(self) -> None:
    ...
    def test_udp_inode_no_pid_returns_neg1(self) -> None:
...
class TestCleanupStaleWireguardConfs:
    def test_removes_dead_pid_conf(self, tmp_path: Path) -> None:
    ...
    def test_keeps_live_pid_conf(self, tmp_path: Path) -> None:
        wg_file = tmp_path / f"wireguard.{os.getpid()}.conf"
        wg_file.write_text('{"private_key": "fake"}')
        _cleanup_stale_wireguard_confs(tmp_path)
        assert wg_file.exists()

    def test_ignores_non_wg_files(self, tmp_path: Path) -> None:
...
class TestKillStaleProcessesExtra:
    @patch("os.kill")
    @patch("time.sleep")
    def test_sends_sigkill_when_still_alive(self, _mock_sleep: Mock, mock_kill: Mock) -> None:
    ...
    @patch("os.kill")
    @patch("time.sleep")
    def test_oserror_logs_error(self, _mock_sleep: Mock, mock_kill: Mock) -> None:
    ...
    @patch("os.kill")
    @patch("time.sleep")
    def test_long_cmdline_snippet(self, _mock_sleep: Mock, mock_kill: Mock) -> None:
        mock_kill.side_effect = ProcessLookupError
        long_cmd = "x" * 200
