...
class TestSilentDropTelemetry:
    """Never-silently-drop diagnostics for the OpenAI Chat render.
    ...
    @staticmethod
    def _run(fsm: OpenAIResponseRenderFSM, events: list[ModelResponseStreamEvent]) -> None:
        loop = asyncio.new_event_loop()
        try:
            for event in events:
    ...
    def test_only_noop_events_emits_no_content_warns(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        fsm = OpenAIResponseRenderFSM(model="gpt-4o")
        events: list[ModelResponseStreamEvent] = [FinalResultEvent(tool_name=None, tool_call_id=None)]
        with caplog.at_level("WARNING", logger="ccproxy.lightllm.graph.openai_render"):
