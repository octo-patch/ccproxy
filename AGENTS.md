...
ccproxy init [--force]                 # Initialize sibling ccproxy.yaml + config.yaml
...
Sibling `ccproxy.yaml` and LiteLLM-compatible `config.yaml` are read from it.
...
`src/ccproxy/templates/ccproxy.yaml` and `src/ccproxy/templates/config.yaml` are generated by
`flake.nix` via `pkgs.formats.yaml.generate` from `defaults.settings` and
`defaults.litellmConfig`, then copied into the repo by the dev shell.
**Do not edit the templates directly**; edit `nix/defaults.nix` and re-enter the dev shell
...
a local pre-commit hook (`sync-ccproxy-template`) that runs the same refresh and stages both templates.
...
`config.providers[name]`; `dest_base_url`/`dest_path` are raw overrides.
...
`.ccproxy/ccproxy.yaml` and `.ccproxy/config.yaml` are **read-only symlinks into the Nix store**.
To change dev settings or model declarations: edit
...
refresh both generated templates from `flake.nix`.
